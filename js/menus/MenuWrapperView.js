...
define(['jquery', 'underscore', 'plist', 'backbone', 'lattice', 'appState', 'threeModel', 'text!menus/templates/MenuWrapperView.html'],
    ...
    var MenuWrapperView = Backbone.View.extend({
        ...
        _setNumber: function($target, newVal){
            ...
            var exclusiveMin = $target.data("min-exclusive");
            var max = $target.data("max");
            var exclusiveMax = $target.data("max-exclusive");
            if (exclusiveMin !== undefined && (exclusiveMin >= newVal)) {
                console.warn("number must be greater than " + exclusiveMin);
                return;
            }
            else if (exclusiveMax !== undefined && (exclusiveMax <= newVal)) {
                console.warn("number must be less than" + exclusiveMax);
                return;
            }
